...
int main(int argc, char **argv)
{
  ...
  slave_vel = node.advertise<geometry_msgs::Twist>("/robot2/cmd_vel", 10);
  ros::Subscriber laser = node.subscribe("/robot2/scan", 1, laserCallBack);
  ...
  while (node.ok())
  {
    ...
    try
    {
      listener.lookupTransform("/robot2", "/robot1", ros::Time(0), transformSM);
    }
    ...
    try
    {
      listener.lookupTransform("/robot1", "/robot2", ros::Time(0), transformMS);
    }
    catch (tf::TransformException &ex)
    {
      ROS_ERROR("%s", ex.what());
    ...
    ROS_INFO("Orientation: [%f]", atan2(transformSM.getOrigin().y(), transformSM.getOrigin().x()));
    ROS_INFO("---------------------------------------------");
    ROS_INFO("Slave pose w.r.t Master [x, y]: [%f, %f]", transformMS.getOrigin().x(), transformMS.getOrigin().y());
    ROS_INFO("Orientation: [%f]", atan2(transformMS.getOrigin().y(), transformMS.getOrigin().x()));
    ...
    vel_msg.linear.x = 0.5 * sqrt(pow(transformSM.getOrigin().x(), 2) + pow(transformSM.getOrigin().y(), 2));
