...
int main(int argc, char **argv)
{
  ...
  slave_vel =
      node.advertise<geometry_msgs::Twist>("/robot3/cmd_vel", 10);
  ros::Subscriber laser = node.subscribe("/robot3/scan", 1, laserCallBack);
  ...
  while (node.ok())
  {
    ...
    try
    {
      listener.lookupTransform("/robot3", "/robot1",
    ...
    try
    {
      listener.lookupTransform("/robot1", "/robot3",
